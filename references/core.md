...
2. **Plain text as the control interface** (Kernighan & Pike). Stages communicate control state through Markdown, YAML, and JSON. Payloads may use a database, API, media file, or efficient tabular format when appropriate, but a human-readable descriptor names its owner, interface, and provenance.
...
4. **Every output is an inspectable surface** (Horvitz / Shneiderman). Each consequential intermediate output has a surface a human can inspect and, where meaningful, edit before the next stage runs. Required gates block; explicitly low-risk deterministic stages may use a risk-based auto gate.
...
| L4 | `output/`, `runs/`, run artifacts | What am I working with? | product (per-run) | varies |
...
For repeated or audited work, the contract also carries a version and gate. Artifact metadata records the run, status, contract version, timestamps, and input hashes. Read [reliability.md](reliability.md); presence alone never proves approval or freshness.
...
- Entry file: prefer `AGENTS.md` for portability or use the native filename required by the primary agent. If multiple entry files exist, declare one canonical; generate the rest or make them one-line pointers — never maintain twins by hand.
...
A stage's full context — entry + contract + references + inputs — should usually land around 2,000–8,000 tokens. Treat this as a diagnostic range, not a model-independent law. If a stage's context balloons: split the stage, tighten the Inputs list, or push detail down into an L3 file the contract points at but doesn't inline. Measure task quality and auditability rather than optimizing a token count in isolation.
